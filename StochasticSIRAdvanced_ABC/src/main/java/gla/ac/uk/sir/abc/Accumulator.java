...
/**
 * Simple class that accumulates values and return statistics about the stored
 * values.
 */
@EqualsAndHashCode
@ToString
public class Accumulator {
    ...
    /**
     * Empty accumulator object. Values are initialised to 0; getting any
     * statistics from this object will result in a divide by zero exceptions
     * since there are zero elements in the accumulator.
     */
    public Accumulator() {
        sum = 0.0;
        sumSqr = 0.0;
    ...
    /**
     * Initialise the internal values of the accumulator with the argument
     * given.
     *
     * @param value the value to initialise the internal variables.
     */
    public Accumulator(final double value) {
        sum = value;
        sumSqr = value * value;
        product = value;
        sumInv = 1.0 / value;
     ...
     */
    public final Accumulator add(final double value) {
        sum += value;
        sumSqr += (value * value);
     ...
     * @return the current accumulator object.
     */
    public final Accumulator add(final Accumulator value) {
        sum += value.sum;
        sumSqr += value.sumSqr;
    ...
    /**
     * Default sample variance implementation based on the second moment \f$
     * M_n^{(2)} \f$ moment<2>, mean and count. \f[ \sigma_n^2 = M_n^{(2)} -
     * \mu_n^2. \f] where \f[ \mu_n = \frac{1}{n} \sum_{i = 1}^n x_i. \f] is the
     * estimate of the sample mean and \f$n\f$ is the number of samples.
     *
    ...
    /**
     * Obtain the standard deviation (error in the mean) of the set of
     * observables in the accumulator.
     *
    ...
    /**
     * Clear the contents of the accumulator. Sets all the internal counts back
     * to zero.
     */
    ...
    /**
     * Get a tab delimited string of the mean and standard deviation of the
